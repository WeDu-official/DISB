...
class VersioningManager:
    ...
    async def _get_relevant_item_versions(self, db_file: str, root_upload_name: str, relative_path_in_archive: str,
        ...
        # Add debug logging to see what we're querying for
        self.log.debug(
            f"Querying for: root='{root_upload_name}', rel_path='{relative_path_in_archive}', base='{base_filename}'")

        all_entries = await self.db._db_read_sync(db_file, {})
        all_item_versions = [
            e for e in all_entries
            if e.get("root_upload_name") == root_upload_name
               and (e.get("relative_path_in_archive") or "") == (relative_path_in_archive or "")
               and e.get("base_filename") == base_filename
        ]
        self.log.debug(f"Found {len(all_item_versions)} raw entries for query")

        # Log the first few entries to see what we got
        if all_item_versions:
            sample = all_item_versions[0]
            self.log.debug(f"Sample entry keys: {list(sample.keys())}")
            self.log.debug(f"Sample entry part numbers: {[e.get('part_number') for e in all_item_versions[:3]]}")
        ...
        if all_versions_param:
            ...
            result = [entry for entry in sorted_versions if entry.get('version') == version_param]
            self.log.debug(f"Found {len(result)} entries for version {version_param}")
            return result
            ...
            if parsed_start == parsed_end:
                result = [entry for entry in sorted_versions if
                          self.parse_version(entry.get('version', '0.0.0.0')) == parsed_start]
                self.log.debug(f"Found {len(result)} entries for version {start_version_param}")
                return result

            result = [entry for entry in sorted_versions if
                      parsed_start <= self.parse_version(entry.get('version', '0.0.0.0')) <= parsed_end]
            self.log.debug(f"Found {len(result)} entries in version range")
            return result
        else:
            # Default: Fetch all rows belonging to the newest version
            if not sorted_versions:
                return []

            newest_version = sorted_versions[-1].get("version")
            print(sorted_versions)
            for entry in sorted_versions:
                self.log.info(f"VERSION RAW: '{entry.get('version')}'")
            return [
                entry for entry in sorted_versions
                if entry.get("version") == newest_version
            ]
